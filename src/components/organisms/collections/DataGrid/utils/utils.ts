  ...
  return columns
    .map(c => {
      ...
      if (typeof c.grow === 'number') {
        if (c.width) return fit ? `minmax(0, ${c.grow}fr)` : `minmax(${c.width}, ${c.grow}fr)`
        return fit ? `minmax(0, ${c.grow}fr)` : `${c.grow}fr`
      }

      if (c.grow === true) {
        if (c.width) return fit ? 'minmax(0, 1fr)' : `minmax(${c.width}, 1fr)`
        return fit ? 'minmax(0, 1fr)' : '1fr'
      }

      if (c.width) {
        return fit ? `minmax(0, ${c.width})` : c.width
      }
