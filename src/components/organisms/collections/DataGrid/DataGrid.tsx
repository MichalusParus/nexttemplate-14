...
export type DataGridProps<T extends Record<string, unknown> = Record<string, unknown>> =
  StyleProps & {
    ...
    maxHeight?: string
    /** fit mode: shrink to parent width and truncate cell content instead of horizontal scroll */
    fit?: boolean
...
function DataGridComponent<T extends Record<string, unknown> = Record<string, unknown>>(
  ...
  devWarning(
    Boolean(onPageChange) && (count == null || page == null),
    'DataGrid: `onPageChange` requires both `count` and `page` props for server-side pagination.',
  )
  devWarning(
    Boolean(onPageChange) && !getRowId,
    'DataGrid: Server-side pagination without `getRowId` will lose row identity across fetches. Server responses create new objects, so the WeakMap UUID cache misses. Provide `getRowId`.',
  )
  ...
  const gridTemplateColumns = useMemo(
    () => buildGridTemplateColumns(columnsInRow, fit),
    [columnsInRow, fit],
  )
  ...
  const handleSelect = useCallback(
    (row: T) => {
      ...
      setSelectedRows(prev => {
        ...
        const next = selectedIds.has(rowId)
  ...
  return (
    <DataGridProvider value={contextValue as DataGridContextValue<Record<string, unknown>>}>
      <div
        ...
        className={cn(
          'DataGrid',
          'grid w-full min-w-0 overflow-x-auto rounded-md',
