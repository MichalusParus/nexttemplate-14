...
export const GridBody = <T extends Record<string, unknown> = Record<string, unknown>>({
  ...
  const { variant, color, size, columnsInRow, fit } = useDataGridContext()
  ...
  return (
    ...
    >
      <ScrollShadow height={maxHeight} gutter disableHorizontal contentRole="rowgroup">
        {isLoading &&
        ...
        {!isLoading && pagedData.length === 0 && (
          ...
          >
            <div
              role="gridcell"
              aria-colindex={1}
              aria-colspan={columnsInRow.length + (multiselect ? 1 : 0)}
