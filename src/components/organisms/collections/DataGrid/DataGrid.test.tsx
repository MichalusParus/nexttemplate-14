...
import { gridData,gridTestCleanColsDef, gridTestColsDef } from '../../../../../.storybook/helpers'
...
describe('DataGrid', () => {
  describe('Semantics', () => {
    it('renders grid role with aria-label', () => {
      render(
        <DataGrid name="dataGridTest" columns={gridTestCleanColsDef} rows={gridData} />,
      )
      const grid = screen.getByRole('grid')

      expect(grid).toBeInTheDocument()
      expect(grid).toHaveAttribute('aria-label', 'dataGridTest')
      expect(grid.parentElement).toHaveAttribute('id', 'grid-dataGridTest')
    })

    it('className lands on outer container', () => {
      render(
        <DataGrid
          className="className"
          name="dataGridTest"
      ...
      expect(screen.getByRole('grid').parentElement).toHaveClass('className')
    })
    ...
    it('aria-colcount matches leaf columns', () => {
      ...
      expect(screen.getByRole('grid')).toHaveAttribute(
        'aria-colcount',
        String(gridTestCleanColsDef.length),
      )
    })
    ...
    it('aria-colcount adds 1 for multiselect column', () => {
      ...
      expect(screen.getByRole('grid')).toHaveAttribute(
        'aria-colcount',
        String(gridTestCleanColsDef.length + 1),
      )
    })
    ...
    it('empty state renders no-rows row', () => {
      render(
        <DataGrid name="dataGridTest" columns={gridTestCleanColsDef} rows={[]} />,
      )
      const rows = screen.getAllByRole('row')
      const gridcells = screen.getAllByRole('gridcell')

      expect(rows).toHaveLength(2) // 1 header row + 1 empty body row
  ...
  describe('Keyboard', () => {
    ...
    it('Enter activates interaction mode on cell with button', async () => {
      render(
        <DataGrid name="dataGridTest" columns={gridTestColsDef} rows={gridData} />,
      )
      const columnheaders = screen.getAllByRole('columnheader')
    ...
    it('End moves focus to last cell in data row', async () => {
      ...
      expect(lastCell.getAttribute('aria-colindex')).toBe(String(gridTestCleanColsDef.length))
    })
