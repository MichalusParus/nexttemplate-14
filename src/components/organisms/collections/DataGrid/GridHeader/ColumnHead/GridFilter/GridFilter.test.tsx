...
import { gridTestColsDef, JestDataGridProvider } from '../../../../../../../../.storybook/helpers'
...
describe('GridFilter', () => {
  describe('Semantics', () => {
    it('renders filter button with aria attributes', () => {
      render(
        <JestDataGridProvider>
          <GridFilter column={gridTestColsDef[0]} />
        </JestDataGridProvider>,
      )
    ...
    it('className lands on wrapper', () => {
      render(
        <JestDataGridProvider>
          <GridFilter className="className" column={gridTestColsDef[0]} />
        </JestDataGridProvider>,
      )
    ...
    it('aria-expanded true when open', async () => {
      ...
      expect(filterButton).toHaveAttribute('data-selected')
    })
    ...
    it('dropdown renders with filter title', async () => {
      ...
      expect(screen.getByText(`Filter in ${gridTestColsDef[0].label}`)).toBeInTheDocument()
    })

    it('filter icon dimmed when no filter applied', () => {
      render(
        <JestDataGridProvider>
          <GridFilter column={gridTestColsDef[0]} />
        </JestDataGridProvider>,
      ...
      expect(filterIcon).toHaveClass('hover-device:opacity-0')
      expect(filterIcon).not.toHaveClass('opacity-100')
    })

    it('filter icon opacity-100 when filter is applied', () => {
      render(
        <JestDataGridProvider
          filter={{ [gridTestColsDef[0].name]: { operator: FilterOperator.EQUALS, value: 'test' } }}
        >
          <GridFilter column={gridTestColsDef[0]} />
        </JestDataGridProvider>,
      )
    ...
    it('renders text filter', async () => {
      const textColumn = { ...gridTestColsDef[0], filter: { type: 'text' as const } }
      render(
        <JestDataGridProvider>
    ...
    it('renders number filter', async () => {
      const numberColumn = { ...gridTestColsDef[0], filter: { type: 'number' as const } }
      render(
        <JestDataGridProvider>
    ...
    it('renders date filter', async () => {
      const dateColumn = { ...gridTestColsDef[0], filter: { type: 'date' as const } }
      render(
        <JestDataGridProvider>
    ...
    it('renders select filter', async () => {
      ...
      expect(selects[0]).toHaveAttribute('id', selectColumn.name)
    })
