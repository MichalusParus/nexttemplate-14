...
export const GridHeader = ({
  ...
  const { variant, color, columns, fit } = useDataGridContext()
  ...
  return (
    ...
    >
      {columnsByDepth.map((columnsAtThisDepth, depth) => {
        ...
        const isLastRow = depth === maxDepth - 1

        return (
          <div key={depth} role="row" aria-rowindex={depth + 1} style={{ display: 'contents' }}>
            {isFirstRow && handleAll && maxDepth > 1 && (
              <div
            ...
            {isLastRow && handleAll && maxDepth > 1 && (
              <SelectAllHead
                isChecked={allSelected}
                isIndeterminate={isIndeterminate}
                handleAll={handleAll}
