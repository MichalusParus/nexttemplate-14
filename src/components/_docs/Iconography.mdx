import { Meta, Title, IconGallery, IconItem } from '@storybook/addon-docs/blocks';

import {CheckIcon} from '../atoms/icons';
import {ChevronIcon} from '../atoms/icons';
import {DeleteIcon} from '../atoms/icons';
import {DoubleChevronIcon} from '../atoms/icons';
import {EditIcon} from '../atoms/icons';
import {SuccessIcon} from '../atoms/icons';
import {InfoIcon} from '../atoms/icons';
import {WarningIcon} from '../atoms/icons';
import {ErrorIcon} from '../atoms/icons';
import {FilterIcon} from '../atoms/icons';
import {LockIcon} from '../atoms/icons';
import {PlusIcon} from '../atoms/icons';
import {MinusIcon} from '../atoms/icons';
import {ProfileIcon} from '../atoms/icons';
import {RefreshIcon} from '../atoms/icons';
import {SearchIcon} from '../atoms/icons';
import {SettingIcon} from '../atoms/icons';
import {SignInIcon} from '../atoms/icons';
import {SignOutIcon} from '../atoms/icons';
import {XIcon} from '../atoms/icons';   
import {CalendarIcon} from '../atoms/icons';
import {PauseIcon} from '../atoms/icons';
import {OpenEyeIcon} from '../atoms/icons';
import {CloseEyeIcon} from '../atoms/icons';

<Meta title="Iconography" />

# Iconography

<IconGallery>
  <IconItem name="CheckIcon">
    <CheckIcon name="CheckIcon" />
  </IconItem>
  <IconItem name="XIcon">
    <XIcon name="XIcon" />
  </IconItem>
  <IconItem name="PlusIcon">
    <PlusIcon name="PlusIcon" />
  </IconItem>
  <IconItem name="MinusIcon">
    <MinusIcon name="MinusIcon" />
  </IconItem>
  <IconItem name="ChevronIcon">
    <ChevronIcon name="ChevronIcon" />
  </IconItem>
  <IconItem name="DoubleChevronIcon">
    <DoubleChevronIcon name="DoubleChevronIcon" />
  </IconItem>
  <IconItem name="SuccessIcon">
    <SuccessIcon name="SuccessIcon" />
  </IconItem>
  <IconItem name="InfoIcon">
    <InfoIcon name="InfoIcon" />
  </IconItem>
  <IconItem name="WarningIcon">
    <WarningIcon name="WarningIcon" />
  </IconItem>
  <IconItem name="ErrorIcon">
    <ErrorIcon name="ErrorIcon" />
  </IconItem>
  <IconItem name="SearchIcon">
    <SearchIcon name="SearchIcon" />
  </IconItem>
  <IconItem name="RefreshIcon">
    <RefreshIcon name="RefreshIcon" />
  </IconItem>
  <IconItem name="FilterIcon">
    <FilterIcon name="FilterIcon" />
  </IconItem>
  <IconItem name="SettingIcon">
    <SettingIcon name="SettingIcon" />
  </IconItem>
  <IconItem name="LockIcon">
    <LockIcon name="LockIcon" />
  </IconItem>
  <IconItem name="EditIcon">
    <EditIcon name="EditIcon" />
  </IconItem>
  <IconItem name="DeleteIcon">
    <DeleteIcon name="DeleteIcon" />
  </IconItem>
  <IconItem name="CalendarIcon">
    <CalendarIcon name="CalendarIcon" />
  </IconItem>
  <IconItem name="ProfileIcon">
    <ProfileIcon name="ProfileIcon" />
  </IconItem>
  <IconItem name="SignInIcon">
    <SignInIcon name="SignInIcon" />
  </IconItem>
  <IconItem name="SignOutIcon">
    <SignOutIcon name="SignOutIcon" />
  </IconItem>
    <IconItem name="PauseIcon">
    <PauseIcon name="PauseIcon" />
  </IconItem>
    <IconItem name="OpenEyeIcon">
    <OpenEyeIcon name="OpenEyeIcon" />
  </IconItem>
    <IconItem name="CloseEyeIcon">
    <CloseEyeIcon name="CloseEyeIcon" />
  </IconItem>
</IconGallery>
